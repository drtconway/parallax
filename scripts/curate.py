...
from fingerprint import query_range
...
def _parse_bam_segments(bam_bytes: bytes) -> list[dict]:
    """Parse a BAM (as raw bytes) and return one dict per alignment record."""
    import tempfile, os
    segments = []
    with tempfile.NamedTemporaryFile(suffix='.bam', delete=False) as tmp:
        ...
        tmp_path = tmp.name
    try:
        with pysam.AlignmentFile(tmp_path, 'rb', check_sq=False) as bam:
            for rec in bam:
...
@app.get('/api/synteny/{read_name:path}')
def get_synteny(read_name: str):
    assert state is not None
    if state.reads_zip is None:
        raise HTTPException(503, 'reads.zip not available — run merge_curation.py')
    safe = read_name.replace('/', '_').replace(' ', '_')
    plx_entry   = _zip_entry_path(safe, '.plx.bam')
    seeds_entry = _zip_entry_path(safe, '.seeds.bam')

    alignment_segs: list[dict] = []
    seeds_segs:     list[dict] = []

    if state.reads_zip.has(plx_entry):
        alignment_segs = _parse_bam_segments(state.reads_zip.read(plx_entry))
    if state.reads_zip.has(seeds_entry):
        seeds_segs = _parse_bam_segments(state.reads_zip.read(seeds_entry))

    return JSONResponse({
        'read_name': read_name,
        'alignment': alignment_segs,
        'seeds':     seeds_segs,
    })
